...
class IssueActionTest extends TestCase
{
    ...
    public function test_throws_when_enrollment_not_passed(): void
    {
        ...
        $this->expectException(
            EnrollmentNotPassedException::class
        );
    ...
    public function test_reuses_existing_certificate_on_duplicate_enrollment_call(): void
    {
        Storage::fake('local');
        ...
        $firstCertificate = $action($enrollment);
        $secondCertificate = $action($enrollment);

        $this->assertSame(
            $firstCertificate->id,
            $secondCertificate->id
        );

        $this->assertSame(
            $firstCertificate->enrollment_id,
            $secondCertificate->enrollment_id
        );
    }
