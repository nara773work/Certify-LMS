...
class QaReplySeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        QaReply::create([
            ...
            'body' => '私も基本情報に合格した後、そのまま応用情報の勉強を始めました。
                        基本情報は知識を問う問題が中心ですが、応用情報は長文を読んで考える問題や記述式が増えるため、
                        最初は難しく感じました。ただ、基本情報の内容をしっかり理解していれば土台はできているので、
                        午後問題の過去問を繰り返し解くことで徐々に慣れていきました。
                        基本情報に合格した勢いのまま学習を続けるのがおすすめです。',
            'qa_thread_id' => 7
        ]);

        QaReply::create([
            'user_id' => $fixedStudent->id,
            'body' => '私もデータベース問題は苦手でしたが、ER図を紙に書きながら問題を解くようにしたら理解しやすくなりました。
                        最初は時間がかかっても構わないので、テーブル同士の関係を整理してから設問を読むようにすると、
                        解答までの流れが見えやすくなりました。',
            'qa_thread_id' => 8
        ]);

        //受講中ユーザー
        $thread1 = QaThread::find(1);
        QaReply::create([
            'user_id' => $students ->where('id', '!=', $thread1->user_id)->random()->id,
            'body' => '私も最初は苦手でしたが、問題を解くたびにIPアドレスとサブネットマスクを2進数に
                        書き直すようにしたら理解できるようになりました。
                        慣れるまでは時間がかかりますが、繰り返し計算しているうちに
                        「どこがネットワーク部なのか」が自然と分かるようになるのでおすすめです。',
            'qa_thread_id' => 1
        ]);

        $thread2 = QaThread::find(11);
        QaReply::create([
        ...
        QaReply::create([
            'user_id' => $ITCoach->id,
            'body' => 'サブネットマスクは「ネットワーク部」と「ホスト部」を区別するためのものです。
                        まずはIPアドレスとサブネットマスクを2進数に変換し、
                        AND演算を行うことでネットワークアドレスを求められます。
                        試験では「255.255.255.0」や「255.255.255.128」など、
                        よく出るサブネットマスクを暗記しておくと計算がスムーズになります。',
            'qa_thread_id' => 1
        ]);
